...
algorithmname="MS-SSIM_python_pytorch"
...
for file in os.listdir(directorybytes):
    if str(file, "utf-8").endswith(".mat") and os.path.isfile(directorybytes+file) and not os.path.isfile(directoryoutput+algorithmname+"/"+algorithmname+str(file, "utf-8")):
        ...
        ms_ssim_value=float(ms_ssim(deg_img, ref_img))
        savemat(directoryoutput+algorithmname+"/"+algorithmname+str(file, "utf-8"), {'algorithmvalue':ms_ssim_value,'algorithmname':algorithmname,'name':name})
